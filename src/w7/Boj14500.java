...
public class Boj14500 {
    ...
    public static int other(int y, int x) { // ㅗ 모양이 중심이 중점
        ...
        if (y - 1 >= 0 && x - 1 >= 0 && x + 1 < m)
            sum = Math.max(arr[y][x - 1] + arr[y][x] + arr[y][x + 1] + arr[y - 1][x], sum); // ㅗ
        if (y + 1 < n && y - 1 >= 0 && x + 1 < m)
            sum = Math.max(arr[y - 1][x] + arr[y][x] + arr[y + 1][x] + arr[y][x + 1], sum);    // ㅏ
        if (y + 1 < n && x - 1 >= 0 && x + 1 < m)
            sum = Math.max(arr[y][x - 1] + arr[y][x] + arr[y][x + 1] + arr[y + 1][x], sum);    // ㅜ
        if (y + 1 < n && y - 1 >= 0 && x - 1 >= 0)
            sum = Math.max(arr[y - 1][x] + arr[y][x] + arr[y + 1][x] + arr[y][x - 1], sum);    //  ㅓ
        return sum;
    }
}